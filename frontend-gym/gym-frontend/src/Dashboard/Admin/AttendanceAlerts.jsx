  ...
  const sendNotification = async () => {
    ...
    if (!isBulkModal && selectedMember) {
        ...
        for (const channel of notifChannels) {
          if (channel === "WHATSAPP") {
            ...
            if (phone) {
              ...
              successes.push("WHATSAPP");
            } else {
              failures.push("WHATSAPP (Missing phone number)");
            }
          } else {
            const payload = {
              type: channel,
            ...
            try {
              await axios.post(`${BaseUrl}notif/send`, payload, axiosConfig);
              successes.push(channel);
            } catch (err) {
              const errorMsg = err.response?.data?.message || err.message;
              failures.push(`${channel} (${errorMsg})`);
            }
          }
        }
        
        if (failures.length === 0) {
          alert(`Message dispatched successfully via: ${successes.join(", ")}`);
        } else if (successes.length > 0) {
          alert(`Partial Success!\nSent via: ${successes.join(", ")}\nFailed via: ${failures.join(", ")}`);
        } else {
      ...
      try {
          ...
          for (const channel of notifChannels) {
            if (channel !== "WHATSAPP" && m.email) {
              try {
                ...
                failureCount++;
                const errorMsg = err.response?.data?.message || err.message;
                failures.push(`${m.fullName}: ${channel} (${errorMsg})`);
              }
            }
          }
        }
        ...
        if (failureCount === 0) {
          alert(`Bulk messages dispatched successfully to all ${targetMembers.length} members via: ${notifChannels.join(", ")}!`);
        } else if (successCount > 0) {
          alert(`Bulk messages sent with some failures:\nSuccessful sends: ${successCount}\nFailed sends: ${failureCount}\n\nErrors:\n${failures.slice(0, 5).join("\n")}${failures.length > 5 ? "\n...and more" : ""}`);
        } else {
