...
const ReceptionistWalkinMember = () => {
  ...
  const adminId = user?.adminId || user?.id || null;
  ...
  useEffect(() => {
    if (location.state?.leadData) {
      ...
      setConvertedLeadId(lead.id); // Save lead ID to mark as converted later
      setShowAddForm(true);
  ...
  const handleAddMember = async (e) => {
    ...
    if (!newMember.planIds || newMember.planIds.length === 0) {
      alert("Please select at least one plan from the Plans section below the Address!");
      return;
    }
  ...
  const handleEditMember = async (e) => {
    ...
    if (!editMember.planIds || editMember.planIds.length === 0) {
      alert("Please select at least one plan from the Plans section!");
      return;
    }
    ...
    try {
      ...
      const errorMsg = error.response?.data?.message || "Failed to update member. Please try again.";
      alert(errorMsg);
    } finally {
      setEditLoading(false);
    }
                        ...
                        <div>
                          ...
                          >
                            ...
                            {!newMember.interestedIn ? (
                              <option value="" disabled>Please select 'Interested In' first</option>
                              ...
                              getFilteredPlans(newMember.interestedIn).map((plan) => (
                                <option key={plan.id} value={plan.id}>
                                  {plan.name} - ₹{plan.price} ({plan.duration || plan.validityDays} Days)
                                </option>
                              ))
                            )}
                          </select>
