...
const DynamicPage = () => {
  ...
  // ✅ Updated: include fullName, phone, email, gender, address, and dateOfBirth
  ...
  const generateBookingReceipt = async (paymentDetails, selectedPlan, bookingResponse) => {
    try {
      ...
      const memberAddress = paymentDetails?.address || "N/A"; 
      const memberDOB = paymentDetails?.dateOfBirth ? new Date(paymentDetails.dateOfBirth).toLocaleDateString() : "N/A"; 
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    const { upi, fullName, phone, email, gender, address, dateOfBirth } = paymentDetails;
    ...
    if (!address.trim()) {
      setBookingMessage('Please enter your address.');
      return;
    }

    if (!dateOfBirth) {
      setBookingMessage('Please enter your date of birth.');
      return;
    }
          ...
          <div className="row align-items-center">
            ...
            <div className="col-md-6 text-center">
              {settings.logo && (
                ...
                >
                  <img
                    src={settings.logo?.replace("http://localhost:4000", import.meta.env.VITE_BACKEND_URL || "https://gym-backend-production-062c.up.railway.app")}
                    alt="logo"
                    style={{
                      width: "260px",
                      maxWidth: "100%",
            ...
            <Form onSubmit={handlePaymentSubmit}>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e) => setPaymentDetails(prev => ({ ...prev, dateOfBirth: e.target.value }))}
                  required
                  disabled={bookingStatus === 'success'}
                />
              </Form.Group>
              ...
              <Form.Group className="mb-3">
                ...
                <Form.Control
                  ...
                  onChange={(e) => setPaymentDetails(prev => ({ ...prev, address: e.target.value }))}
                  required
                  disabled={bookingStatus === 'success'}
                />
              </Form.Group>
